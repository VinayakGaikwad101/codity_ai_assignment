...
async function main() {
    ...
    // 2. Create Users (RBAC)
    const saltRounds = 10;
    const adminPasswordHash = await bcrypt_1.default.hash('Admin@12345', saltRounds);
    const operatorPasswordHash = await bcrypt_1.default.hash('Operator@12345', saltRounds);
    const viewerPasswordHash = await bcrypt_1.default.hash('Viewer@12345', saltRounds);
    ...
    const hashedApiKey = crypto_1.default.createHash('sha256').update(rawApiKey).digest('hex');
    ...
    const expRetryPolicy = await prisma.retryPolicy.create({
        data: {
            ...
            strategy: client_1.RetryStrategy.EXPONENTIAL,
            maxRetries: 3,
            baseDelayMs: 2000,
            maxDelayMs: 60000,
            jitter: true,
    ...
    const linearRetryPolicy = await prisma.retryPolicy.create({
        data: {
            ...
            strategy: client_1.RetryStrategy.LINEAR,
            maxRetries: 5,
            baseDelayMs: 1000,
            maxDelayMs: 30000,
            jitter: true,
    ...
    const fixedRetryPolicy = await prisma.retryPolicy.create({
        data: {
            ...
            strategy: client_1.RetryStrategy.FIXED,
            maxRetries: 2,
            baseDelayMs: 5000,
            maxDelayMs: 5000,
            jitter: false,
    ...
    const cronJob1 = await prisma.scheduledJob.create({
        data: {
            ...
            handlerType: client_1.JobHandlerType.CUSTOM_COMPUTE,
            cronExpression: '0 * * * *',
            timezone: 'UTC',
    ...
    const cronJob2 = await prisma.scheduledJob.create({
        data: {
            ...
            handlerType: client_1.JobHandlerType.SAMPLE_REPORT,
            cronExpression: '0 0 * * *',
            timezone: 'UTC',
    ...
    await prisma.job.create({
        data: {
            ...
            handlerType: client_1.JobHandlerType.SAMPLE_EMAIL,
            jobType: client_1.JobType.IMMEDIATE,
    ...
    await prisma.job.create({
        data: {
            ...
            handlerType: client_1.JobHandlerType.HTTP_WEBHOOK,
            jobType: client_1.JobType.IMMEDIATE,
            status: client_1.JobStatus.QUEUED,
            priority: 50,
